...
<div class="container my-5">
	...
	<div class="mt-4">
		<form method="post" action="{{ route('pengajuan.legalisasi.store') }}">
			@csrf
			<div>
				...
				<div class="d-md-flex align-items-center mt-3">
					...
					<div class="col-md-7 position-relative d-flex align-items-center">
						<label class="d-flex align-items-center">
							<input disabled="" @if($data['pengambilan'] == 'Ambil di kampus') checked @endif class="d-none" type="radio" name="pengambilan" value="Ambil di kampus">
							<div class="p-2 me-2 rounded-circle border border-secondary"></div>
							Ambil di kampus
						</label>
						<label class="d-flex align-items-center">
							<input @if($data['pengambilan'] == 'Dikirim') checked @endif disabled="" class="d-none" type="radio" name="pengambilan" value="Dikirim">
							<div class="p-2 me-2 ms-2 rounded-circle border border-secondary"></div>
							Dikirim
						</label>
					</div>
				</div>
				<div @if($data['pengambilan'] == 'Ambil di kampus') class="d-none" @endif  id="setDikirim">
					<div class="d-md-flex mt-3">
						<label class="col-md-2"><strong>Alamat Pengiriman <em class="text-danger">*</em></strong></label>
						<div class="col-md-7 position-relative">
							<textarea @if($data['pengambilan'] == 'Dikirim') required="" @endif  placeholder="Tulis Alamat Tujuan disini" class="form-control" name="alamat" rows="7"></textarea>
						</div>
					</div>
					<div class="d-md-flex mt-3">
						<label class="col-md-2"><strong>Kode pos <em class="text-danger">*</em></strong></label>
						<div class="col-md-7 position-relative">
							<input @if($data['pengambilan'] == 'Dikirim') required="" @endif placeholder="Tulis Kode Pos disini" class="form-control" name="kodepos" type="number">
						</div>
					</div>
				</div>
				<div class="text-end col-md-9 mt-3">
					<button type="submit" name="tambah" class="btn bg-purple text-white">Pesan Legalisasi</button>
				</div>
			</div>
		</form>
	</div>
